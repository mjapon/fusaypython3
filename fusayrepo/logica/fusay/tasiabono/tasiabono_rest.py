...
from fusayrepo.logica.fusay.tasiabono.abonos_proveedores import AbonosProveedoresService
...
class TAsiAbonoRest(TokenView):
    ...
    def collection_post(self):
        ...
        elif accion == 'abonar_creditos_prov':
            form = self.get_request_json_body()
            service = AbonosProveedoresService(self.dbsession)
            service.crear_abonos(formdata=form, sec_codigo=self.get_sec_id(), usercrea=self.get_user_id())
            msg = 'Abonos registrados exitosamente'
            return self.res200({'msg': msg})
