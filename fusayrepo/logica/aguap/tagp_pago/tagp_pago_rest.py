...
@resource(collection_path='/api/tagpcobro', path='/api/tagpcobro/{mdg_id}')
class TagpCobroRest(TokenView):
    ...
    def collection_post(self):
        ...
        if accion == 'gcalpag':
            ...
            if 'montos' in form:
                montos = form['montos']
                if 'pagosdet' in montos:
                    pagosdet = montos['pagosdet']
                    for key in pagosdet.keys():
                        parsedpagosdet[int(str(key))] = pagosdet[key]
                    form['montos']['pagosdet'] = parsedpagosdet
